...
class PenugasanGuruController extends GetxController {
  ...
  Future<void> removeGuruFromMapel(String mapelId) async {
    ...
    try {
      ...
      final pegawaiJadwalRef = _firestore.collection('Sekolah').doc(configC.idSekolah).collection('pegawai').doc(guruId).collection('jadwal_mengajar').doc(tahunAjaranAktif).collection('mapel_diampu').doc(mapelId);
