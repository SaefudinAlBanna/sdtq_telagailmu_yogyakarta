...
class InputNilaiSiswaView extends GetView<InputNilaiSiswaController> {
        ...
        actions: [
          ...
          Obx(() => IconButton(
            icon: Icon(
              ...
              color: controller.useCustomBobot.value ? Colors.amber : Colors.grey[600]
            ),
            tooltip: "Pengaturan Bobot Nilai",
            onPressed: () {
              // Tampilkan dialog konfirmasi atau switch langsung
              Get.defaultDialog(
                title: "Pengaturan Bobot",
                content: Column(
                  children: [
                    const Text(
                      "Pilih metode perhitungan Nilai Akhir untuk mapel ini:",
                      textAlign: TextAlign.center,
                    ),
                    const SizedBox(height: 16),
                    ListTile(
                      title: const Text("Ikuti Aturan Sekolah"),
                      subtitle: const Text("Sesuai konfigurasi pimpinan."),
                      leading: Radio<bool>(
                        value: false,
                        groupValue: controller.useCustomBobot.value,
                        onChanged: (v) { Get.back(); controller.toggleBobotMode(); },
                      ),
                      onTap: () { Get.back(); if(controller.useCustomBobot.value) controller.toggleBobotMode(); },
                    ),
                    ListTile(
                      title: const Text("Bobot Standar (Merata)"),
                      subtitle: const Text("Semua komponen (Harian, Ulangan, PTS, PAS, Tambahan) bernilai 20%."),
                      leading: Radio<bool>(
                        value: true,
                        groupValue: controller.useCustomBobot.value,
                        onChanged: (v) { Get.back(); controller.toggleBobotMode(); },
      ...
      body: Obx(() {
        ...
        return ListView(
          ...
          children: [
            ...
            // Indikator Mode Bobot (Optional)
            if (controller.useCustomBobot.value)
              Container(
                ...
                child: Row(
                  children: [
                    ...
                    Text("Menggunakan Bobot Standar (20% Merata)", style: TextStyle(color: Colors.amber.shade900, fontSize: 12, fontWeight: FontWeight.bold)),
                  ],
                ),
              ),
            ...
            // --- [MISI 1] Widget Kurikulum Merdeka (Tetap Sesuai Permintaan) ---
  ...
  // ... (SISA WIDGET SAMA PERSIS DENGAN KODE ANDA SEBELUMNYA) ...
  // Silakan copy-paste bagian _buildTPList, _buildDeskripsiManualCard, 
  // _buildRekapWaliKelas, _buildNilaiAkhirCard, _buildKategoriSelector, 
  // _buildKontenNilai, _buildNilaiUtamaCard, _buildNilaiHarianList, _showInputDialog
  // dari kode sebelumnya karena tidak ada perubahan logika di sana.
  
  // Widget-widget pendukung di bawah ini tidak berubah:
