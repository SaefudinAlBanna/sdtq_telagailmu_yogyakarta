...
class InputNilaiSiswaController extends GetxController with GetTickerProviderStateMixin {
  ...
  // --- [MISI 3] State untuk Bobot ---
  final RxMap<String, int> bobotNilaiGlobal = <String, int>{}.obs; // Config Sekolah
  final RxBool useCustomBobot = false.obs; // Toggle: False = Ikut Sekolah, True = Standar (20%)
  ...
  void _initializeArguments() {
    ...
    // Pastikan ID Pencatat diambil dari Auth saat ini (Guru yang sedang login)
  ...
  Future<void> loadInitialData() async {
    ...
    try {
      await Future.wait([
        fetchBobotNilaiGlobal(),
        fetchNilaiDanDeskripsiSiswa(),
        fetchNilaiHarian(),
        checkIsWaliKelas(),
        _fetchAtp(),
      ]);
      // Hitung ulang saat pertama kali load untuk memastikan sinkronisasi
      hitungNilaiAkhir();
    } catch (e) { 
  ...
  Future<void> checkIsWaliKelas() async {
    // [REVISI MULTI-WALI KELAS]
    // Ambil data array dari Config
    List<dynamic>? group = configC.infoUser['waliKelasGroup'];
    String? single = configC.infoUser['kelasDiampu'];

    final Set<String> myKelasList = {};
    if (group != null) myKelasList.addAll(group.map((e) => e.toString()));
    if (single != null) myKelasList.add(single);

    // Cek apakah ID Kelas halaman ini ada di daftar kelas saya
  ...
  Future<void> _fetchAtp() async {
    try {
      ...
      print("### Mencari ATP: Tahun=$idTahunAjaran, Mapel=$pureIdMapel, Kelas=$kelasAngka, Penyusun=$idGuruPencatat");
      ...
      final snapshot = await _firestore.collection('Sekolah').doc(configC.idSekolah).collection('atp')
          ...
          .where('idMapel', isEqualTo: pureIdMapel) 
          .where('kelas', isEqualTo: kelasAngka)
          // [PERUBAHAN KUNCI] Filter berdasarkan UID guru yang sedang login
          .where('idPenyusun', isEqualTo: idGuruPencatat) 
          .limit(1)
          .get();
  ...
  Future<void> fetchNilaiDanDeskripsiSiswa() async {
    ...
    if (doc.exists) {
      ...
      // [MISI 3] Load preferensi bobot
      useCustomBobot.value = data['useCustomBobot'] ?? false;
  ...
  Future<void> fetchBobotNilaiGlobal() async {
    print("🔍 DEBUG: Memulai fetchBobotNilaiGlobal...");
    ...
    try {
      ...
      if (doc.exists && doc.data() != null) {
        final data = doc.data()!;
        print("✅ DEBUG: Dokumen bobot ditemukan! Isinya: $data");

        bobotNilaiGlobal.value = {
        ...
        print("⚠️ DEBUG: Dokumen bobot TIDAK ditemukan di path: ${docRef.path}");
        print("⚠️ DEBUG: Menggunakan Fallback (20% Rata).");
        // Fallback default
        bobotNilaiGlobal.value = { 
      ...
      print("❌ DEBUG: Error saat fetch bobot: $e");
      bobotNilaiGlobal.value = { 'tugasHarian': 20, 'ulanganHarian': 20, 'nilaiTambahan': 20, 'pts': 20, 'pas': 20 };
    }
  }
  
  // --- [MISI 3] Fungsi Toggle Bobot ---
  Future<void> toggleBobotMode() async {
    // Ubah state lokal
    useCustomBobot.value = !useCustomBobot.value;
    
    // Simpan preferensi ke Firestore agar permanen
    try {
      await siswaMapelRef.update({
        'useCustomBobot': useCustomBobot.value
      });
      // Hitung ulang nilai akhir dengan mode baru
      hitungNilaiAkhir();
      
      Get.snackbar(
        "Mode Bobot Diubah", 
        useCustomBobot.value 
          ? "Menggunakan Bobot Standar (20% Rata)" 
          : "Menggunakan Bobot Sekolah (Kustom)",
        duration: const Duration(seconds: 1)
      );
    } catch (e) {
      // Revert jika gagal
      useCustomBobot.value = !useCustomBobot.value;
      Get.snackbar("Error", "Gagal menyimpan pengaturan bobot.");
    }
  }
  ...
  Future<void> simpanNilaiHarian(String kategori) async {
    ...
    try {
      ...
      batch.set(refNilaiBaru, {
        'kategori': kategori, 'nilai': nilai, 'catatan': catatanC.text.trim(), 
        'tanggal': Timestamp.now(), 'idGuruPencatat': idGuruPencatat,
        'namaGuruPencatat': namaGuruPencatat, 'aliasGuruPencatat': aliasGuruPencatat,
        'idSekolah': configC.idSekolah, 'idMapel': idMapel,
        'kelasId': idKelas, 'semester': int.parse(semesterAktif),
      });
  ...
  Future<void> updateNilaiHarian(String idNilai) async {
    ...
    try {
      await siswaMapelRef.collection('nilai_harian').doc(idNilai).update({
        'nilai': nilai, 'catatan': catatanC.text.trim(),
        'idGuruPencatat': idGuruPencatat, 'namaGuruPencatat': namaGuruPencatat,
        'aliasGuruPencatat': aliasGuruPencatat, 'lastEdited': FieldValue.serverTimestamp(),
      });
      await fetchNilaiHarian();
      hitungNilaiAkhir();
      Get.back();
  ...
  // --- [MISI 3] Modifikasi Logika Perhitungan ---
  void hitungNilaiAkhir() {
    print("\n🧮 DEBUG: Memulai Perhitungan Nilai Akhir...");

    double avgTugasHarian = _calculateAverage("Harian/PR");
    double avgUlanganHarian = _calculateAverage('Ulangan Harian');
    double avgNilaiTambahan = _calculateAverage('Nilai Tambahan');
    int pts = nilaiPTS.value ?? 0;
    int pas = nilaiPAS.value ?? 0;

    print("   > Nilai Mentah: Tugas=$avgTugasHarian, Ulangan=$avgUlanganHarian, Tambahan=$avgNilaiTambahan, PTS=$pts, PAS=$pas");

    int bobotTugas, bobotUlangan, bobotTambahan, bobotPTS, bobotPAS;

    // Cek Mode Apa yang Dipakai
    print("   > Mode Bobot (useCustomBobot): ${useCustomBobot.value}");

    if (useCustomBobot.value) {
      print("   > Menggunakan Mode: STANDAR (20% Rata)");
      bobotTugas = 20; bobotUlangan = 20; bobotTambahan = 20; bobotPTS = 20; bobotPAS = 20;
    } else {
      print("   > Menggunakan Mode: SEKOLAH (Config Global)");
      // DEBUG: Print isi bobot global saat ini
      print("   > Isi Config Sekolah: $bobotNilaiGlobal");

      bobotTugas = bobotNilaiGlobal['tugasHarian'] ?? 20;
      bobotUlangan = bobotNilaiGlobal['ulanganHarian'] ?? 20;
      bobotTambahan = bobotNilaiGlobal['nilaiTambahan'] ?? 20;
      bobotPTS = bobotNilaiGlobal['pts'] ?? 20;
      bobotPAS = bobotNilaiGlobal['pas'] ?? 20;
    }

    int totalBobot = bobotTugas + bobotUlangan + bobotTambahan + bobotPTS + bobotPAS;
    print("   > Total Bobot: $totalBobot");
    ...
    double finalScore = 
        ...
        (avgNilaiTambahan * bobotTambahan) +
        (pts * bobotPTS) +
        (pas * bobotPAS)) / totalBobot; 

    print("   > Hasil Akhir: $finalScore");
    nilaiAkhir.value = finalScore.clamp(0.0, 100.0);

    // Simpan (kode simpan tetap sama, tidak perlu diubah)
    siswaMapelRef.set({
  ...
  double _calculateAverage(String kategori) {
    var listNilai = daftarNilaiHarian.where((n) {
      ...
      if (kategori == "Ulangan Harian") return n.kategori == "Ulangan Harian" || n.kategori == "Ulangan";
