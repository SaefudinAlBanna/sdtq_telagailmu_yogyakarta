...
class HalaqahManagementController extends GetxController {
  ...
  Future<void> _fetchGroups() async {
    ...
    try {
      ...
      // --- [STRATEGI BARU] Ambil data profil terbaru ---
      // 1. Kumpulkan semua ID pengampu yang unik dari grup yang ada.
      final pengampuIds = groupSnapshot.docs
          .map((doc) => doc.data()['idPengampu'] as String)
          .toSet()
          .toList();

      // 2. Ambil data profil semua pengampu dalam satu query 'whereIn'.
      final pengampuProfilesSnapshot = await _firestore
          .collection('Sekolah').doc(configC.idSekolah)
          .collection('pegawai')
          .where(FieldPath.documentId, whereIn: pengampuIds)
          .get();

      // 3. Buat Peta (Map) untuk akses cepat: 'idPengampu' -> 'urlGambarProfil'
      final Map<String, String?> profileUrlMap = {
        for (var doc in pengampuProfilesSnapshot.docs)
      ...
      for (var doc in groupSnapshot.docs) {
        ...
        groupsWithCount.add(HalaqahGroupWithCount(
          ...
          // [MODIFIKASI] Gunakan URL terbaru dari Peta yang kita buat.
          // Jika tidak ada di peta (jarang terjadi), gunakan data lama dari grup.
          profileImageUrl: profileUrlMap[groupData.idPengampu] ?? groupData.profileImageUrl,
  ...
  void goToCreateEditGroup([HalaqahGroupModel? group]) {
    HalaqahGroupModel? argumentToSend;

    if (group != null) {
      // [PERBAIKAN KUNCI] Buat instance baru dari tipe dasar (HalaqahGroupModel)
      // Ini memastikan tidak ada ambiguitas tipe data subclass saat meneruskan argumen.
      argumentToSend = HalaqahGroupModel(
        id: group.id,
        namaGrup: group.namaGrup,
        idPengampu: group.idPengampu,
        namaPengampu: group.namaPengampu,
        aliasPengampu: group.aliasPengampu,
        semester: group.semester,
        profileImageUrl: group.profileImageUrl,
      );
    }

    // Kirim argumen yang sudah "dibersihkan"
      Get.toNamed(Routes.CREATE_EDIT_HALAQAH_GROUP, arguments: argumentToSend)?.then((_) {
        // Callback untuk refresh list tetap sama
        _fetchGroups(); 
      });
  }
